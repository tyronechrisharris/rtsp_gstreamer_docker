...
REM # This script is pre-configured to run the server on port 554 with the H.264 codec.
...
set DOCKER_RUN_CMD=docker run -it --rm --name rtsp-h264-554-stream %ENV_VARS% -p %RTSP_SERVER_PORT%:%RTSP_SERVER_PORT%/tcp -p %RTSP_SERVER_PORT%:%RTSP_SERVER_PORT%/udp %DOCKER_IMAGE_NAME%
